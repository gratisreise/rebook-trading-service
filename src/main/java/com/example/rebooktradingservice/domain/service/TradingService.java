...
import com.example.rebooktradingservice.clientfeign.book.BookClient;
...
import com.example.rebooktradingservice.external.s3.S3Service;
...
public class TradingService {
    ...
    public void postTrading(TradingRequest request, String userId, MultipartFile file) throws IOException {
        ...
        NotificationTradeMessage message = new NotificationTradeMessage(trading.getId(), content, request.bookId());
        saveOutBox(message);
    }
    ...
    public void updateTrading(TradingRequest request, String userId, Long tradingId, MultipartFile file)
        ...
        if (request.price() != trading.getPrice()) {
            String content = "찜한 제품의 가격이 변동되었습니다.";
            NotificationTradeMessage message =
                new NotificationTradeMessage(tradingId, content, request.bookId());
            saveOutBox(message);
        }
